		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
				    <span :style='{"border":"1px solid rgba(0,0,0,0.3)","padding":"0 10px","boxShadow":"0 0 6px rgba(0,0,0,0.3)","borderRadius":"4px","color":"#fff","background":"rgba(0,0,0,0.4)","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.content"></span>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
				    <span :style='{"border":"1px solid rgba(0,0,0,0.3)","padding":"0 10px","boxShadow":"0 0 6px rgba(0,0,0,0.3)","borderRadius":"4px","color":"#fff","background":"rgba(0,0,0,0.4)","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.reply"></span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusszhaopinzixun/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discusszhaopinzixun/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusszhaopinzixunCrossAddOrUpdateFlag = false;
											this.parent.search();
